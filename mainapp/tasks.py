...
""" If the mailing end date has not arrived, we send a message, change the status to "submitted" in the database """
...
def send_message(message_id: int, client: dict, data: dict, end_date: str):
    ...
    if datetime.datetime.now() < end_date:
        ...
        if time_interval_start <= local_time_now <= time_interval_end:
            request = requests.post(f"https://probe.fbrq.cloud/v1/send/{message_id}",json={"id": message_id, "phone": client.get("phone"),"text": data.get("message_text")},headers={"Authorization": env('TOKEN')}, timeout=60)
        ...
        elif local_time_now < time_interval_start:
            ...
            send_message.apply_async(args=(message_id, client, data, end_date),countdown=date_time_difference.total_seconds() + 60)
            ...
            send_message.apply_async(args=(message_id, client, data, end_date),countdown=total_countdown)
...
def prepare_mailing(data: dict, end_date: str):
    ...
    for client in clients.values('id', 'phone'):
        message = Message.objects.create(message_sent_date=timezone.now(),mailing_id=data.get('id'),client_id=client.get('id'))
